...
class AntColonyOptimizer:
    ...
    def distance_to_line(self, point, line_start, line_end):
        ...
        return self.calculate_distance(point, proj)

    def initialize_pheromone(self, start_coord, end_coord):
        """根据启发式方法初始化信息素。"""
        for edge in self.graph.edges:
            node1_coord = self.parse_coord(self.graph.nodes[edge[0]]['coord'])
            node2_coord = self.parse_coord(self.graph.nodes[edge[1]]['coord'])
            edge_midpoint = self.midpoint(node1_coord, node2_coord)
            distance = self.distance_to_line(edge_midpoint, start_coord, end_coord)
            # 调整信息素值，示例中使用简化的逆比例关系
            pheromone_value = max(100 / (1 + distance), 10)  # 距离越近，信息素值越高
            self.pheromone[edge] = pheromone_value
